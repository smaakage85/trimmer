...
`daTrimmer` is compatible with all R objects, that inherit from the `list`
class - not just R model objects - and all kinds of functions - not just the
`predict function`. Hence `daTrimmer` is quite a flexible tool.
...
That is it, I hope, that you will enjoy the `daTrimmer` package :)